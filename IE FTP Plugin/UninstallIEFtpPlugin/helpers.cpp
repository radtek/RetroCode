...
CString	MyGetProfileString(CString strTag, CString strDefault, DWORD dwMaxChars)
{
	...
	if(reg.Open(HKEY_LOCAL_MACHINE, MY_REGISTRY_PATH, KEY_READ) == ERROR_SUCCESS)
	{
		 ...
		 if(!bOK)
		 {
			 ATLTRACE("Failed to read registry error: %d\r\n",GetLastError());		
...
BOOL UnRegisterOCX(CString strPath)
{
	...
	// remove registry as well if exists
	BOOL bOK = false;
	CRegKey reg;
	if(reg.Create(HKEY_LOCAL_MACHINE, "SOFTWARE\\Microsoft\\Internet Explorer\\Toolbar") == ERROR_SUCCESS)
	{
		 bOK = (reg.DeleteValue("{1FFFE263-3C8E-4DD8-9BAA-36998D29561E}") == ERROR_SUCCESS);
		 reg.Close();
	}  
