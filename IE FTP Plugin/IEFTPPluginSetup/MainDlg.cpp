...
LRESULT CMainDlg::OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
	...
	CString strDir = strReturn + "\\";
	strDir += FOLDERNAME_INSTALL_PATH;
	strDir.Replace("\\\\","\\");
    ...
    if (::SHGetMalloc(&pMalloc) == NOERROR)
    {
        ...
        if ((pidl = ::SHBrowseForFolder(&bi)) != NULL)
        {
            ...
            { 
				...
				CString strTest = strText.Right(strlen(FOLDERNAME_INSTALL_PATH));
				if(strTest.CompareNoCase(FOLDERNAME_INSTALL_PATH) != 0)	
				{
					strText += "\\";
					strText += FOLDERNAME_INSTALL_PATH;
				}
...
{
	m_nPage = 1;
	m_strTitleText.Format("Welcome to the %s Setup Wizard",MY_APP_NAME);
